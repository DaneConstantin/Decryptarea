...
const DescriptionText = ({ article }) => {
    const isFromCoinTelegraph = article.description[0]?.includes('<p>');

    // Strip HTML tags if the article is from CoinTelegraph
    const descriptionText = isFromCoinTelegraph ? stripHtmlTags(article.description[0]) : article.description[0];
